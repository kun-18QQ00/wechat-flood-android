...
class MsgApp(App):
    ...
    service_text = StringProperty('检查中...')
    ...
    def _check_service(self):
        ...
        try:
            ...
            if inst:
                ...
                self.service_text = '无障碍已连接'
                self._log('无障碍服务已连接 - 可全自动发送')
            else:
                self._use_accessibility = False
                self.service_text = '无障碍未开启'
                self._log('无障碍未开启 - 使用剪贴板模式')
        except Exception as e:
            self._use_accessibility = False
            self.service_text = '检查失败'
    ...
    def start(self):
        ...
        svc = self._get_svc()
        if svc:
            self._log(f'全自动模式 | {len(self.messages)}条 | {self.interval}秒')
        else:
            self._log(f'剪贴板模式 | {len(self.messages)}条 | {self.interval}秒')
            self._log('请切到聊天窗口，消息会自动复制')
    ...
    def _on_sent(self, msg, auto):
        ...
        self._update_stats()
        t = msg[:10] + '..' if len(msg) > 10 else msg
        tag = '自动发送' if auto else '已复制'
    ...
    def on_resume(self):
        Clock.schedule_once(lambda dt: self.refresh_service(), 1)
